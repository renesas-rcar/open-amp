			     ...
			     uint32_t src, void *priv)
{
	...
	rvdev = metal_container_of(ept->rdev, struct rpmsg_virtio_device, rdev);
	io = rvdev->shbuf_io;

	metal_io_block_read(io, metal_io_virt_to_offset(io, data), payload, len);

	LPRINTF("Incoming message %d: %s\r\n", rnum + 1, payload);
